...
export const addPost = async (req, res) => {
    ...
    if (!req.file) {
        ...
        const post = await postModel.create({ title, caption, userId: req.user._id, image: secure_url })
        res.redirect("/post")
    }
}
...
export const deletePost = async (req, res) => {
    const { id } = req.params
    const post = await postModel.findOneAndDelete({ _id: id, userId: req.user._id })
    res.redirect("/post")
...
export const updatePost = async (req, res) => {
    ...
    if (!post) {
        ...
        const post = await postModel.updateOne({ _id: id, userId: req.user._id }, { title, caption, image: imageURl })
        res.redirect("/post")
    }
